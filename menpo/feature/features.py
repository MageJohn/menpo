...
from .base import imgfeature, ndfeature, winitfeature
...
def gaussian_filter(pixels, sigma):
    ...
    from scipy.ndimage import \
        gaussian_filter as scipy_gaussian_filter  # expensive
...
def hog(
    ...
    if mode == "dense":
        ...
        if (
            window_height_temp < block_size * cell_size
            or window_height_temp > pixels.shape[0]
        ):
            raise ValueError(
                "Window height must be >= block size and <= " "image height"
            )
        if (
            window_width_temp < block_size * cell_size
            or window_width_temp > pixels.shape[1]
        ):
            raise ValueError("Window width must be >= block size and <= " "image width")
    ...
    # Sparse case
        ...
        if algorithm == "dalaltriggs":
            ...
            step = cell_size
        iterator = WindowIterator(pixels, window_size, window_size, step, step, False)
    # Print iterator's info
    if verbose:
        print(iterator)
    # Compute HOG
    hog_descriptor = iterator.HOG(
        algorithm,
        num_bins,
        cell_size,
        block_size,
...
def lbp(
    ...
    if not skip_checks:
        ...
        if mapping_type not in ["u2", "ri", "riu2", "none"]:
            raise ValueError("Mapping type must be u2, ri, riu2 or " "none")
    ...
    if window_step_unit == "window":
        ...
        window_step_horizontal = np.uint32(window_step_horizontal * window_width)
    if mapping_type == "u2":
        mapping_type = 1
